...
router.get("/document-print-settings", (_req, res) => {
  ...
  res.json({
    ...
    logoUrl: row.logo_url && String(row.logo_url).trim() !== "" && row.logo_url !== "null" ? row.logo_url : "/assets/images/omnisystem_pro_logo_1784250216808.png",
...
router.put("/document-print-settings", (req, res) => {
  ...
  const existingRow = db.prepare("SELECT logo_url FROM document_print_settings WHERE id = 1").get() as any;
  const currentLogoUrl = existingRow?.logo_url || "/assets/images/omnisystem_pro_logo_1784250216808.png";
