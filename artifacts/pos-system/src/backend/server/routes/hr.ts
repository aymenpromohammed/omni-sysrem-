...
/* ── Employees ── */
router.get("/hr/employees/next-number", (req, res) => {
  if (!requireAdmin(req, res)) return;
  const rows = db.prepare("SELECT employee_number FROM hr_employees").all() as any[];
  let maxNum = 0;
  for (const r of rows) {
    if (!r.employee_number) continue;
    const nums = String(r.employee_number).replace(/\D/g, "");
    if (nums) {
  ...
  res.json({ next_number: `EMP-${String(nextVal).padStart(3, "0")}` });
});
...
router.get("/hr/employees", (req, res) => {
  ...
  const rows = db.prepare(`
    ...
    ORDER BY e.employee_number ASC, e.name ASC
  `).all() as any[];
...
router.post("/hr/employees", (req, res) => {
  ...
  if (!name) { res.status(400).json({ error: "اسم الموظف مطلوب" }); return; }
  
  if (!employee_number || String(employee_number).trim() === "") {
    const rows = db.prepare("SELECT employee_number FROM hr_employees").all() as any[];
    let maxNum = 0;
    for (const r of rows) {
      if (!r.employee_number) continue;
    ...
    employee_number = `EMP-${String(maxNum + 1).padStart(3, "0")}`;
  } else {
    employee_number = String(employee_number).trim();
  }

  // Check duplicate
  const existing = db.prepare("SELECT id, name FROM hr_employees WHERE employee_number=?").get(employee_number) as any;
  if (existing) {
    res.status(400).json({ error: `رقم الموظف (${employee_number}) مستخدم بالفعل للموظف (${existing.name})` });
    return;
  }
...
router.put("/hr/employees/:id", (req, res) => {
  ...
  // Check duplicate
  const existing = db.prepare("SELECT id, name FROM hr_employees WHERE employee_number=? AND id!=?").get(employee_number, req.params.id) as any;
  if (existing) {
    res.status(400).json({ error: `رقم الموظف (${employee_number}) مستخدم بالفعل للموظف (${existing.name})` });
    return;
  }
