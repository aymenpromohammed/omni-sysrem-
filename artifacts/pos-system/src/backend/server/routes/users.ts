...
router.get("/users", (req, res) => {
  ...
  let rows = db.prepare("SELECT id, username, name, role, active, can_discount FROM users ORDER BY name").all() as any[];
...
router.post("/users", (req, res) => {
  const user = getAuthUser(req);
  if (!user || (user.role !== "admin" && user.role !== "developer")) { res.status(403).json({ error: "غير مصرح" }); return; }
  const { username, name, role, password, active, can_discount } = req.body;
  ...
  const r = db.prepare("INSERT INTO users (username, password_hash, name, role, active, can_discount) VALUES (?,?,?,?,?,?)").run(username, hash, name, role, active !== false ? 1 : 0, discountFlag);
  const u = db.prepare("SELECT id, username, name, role, active, can_discount FROM users WHERE id=?").get(r.lastInsertRowid) as any;
...
router.put("/users/:id", (req, res) => {
  ...
  if (password) {
    const hash = hashPassword(password);
    db.prepare("UPDATE users SET username=?, name=?, role=?, password_hash=?, active=?, can_discount=? WHERE id=?").run(username, name, targetRole, hash, active !== false ? 1 : 0, discountFlag, req.params.id);
  } else {
    db.prepare("UPDATE users SET username=?, name=?, role=?, active=?, can_discount=? WHERE id=?").run(username, name, targetRole, active !== false ? 1 : 0, discountFlag, req.params.id);
  }
  const u = db.prepare("SELECT id, username, name, role, active, can_discount FROM users WHERE id=?").get(req.params.id) as any;
  res.json(toUser(u));
});
