...
export default function SuppliersPage() {
  ...
  const { data: reportData = [] } = useQuery({
    queryKey: ["procurement-report", selectedReportType],
    queryFn: () => apiGet(`/api/purchases/reports?type=${selectedReportType}`),
      ...
      <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-4 rounded-xl border">
              <div>
                <h2 className="text-base font-bold flex items-center gap-2">
                  <BarChart3 className="w-5 h-5 text-primary" /> التقارير الشاملة للمشتريات والموردين (12 تقرير متقدم)
                </h2>
                <p className="text-xs text-muted-foreground">اختر نوع التقرير لعرض التحليلات والجداول المالية التفصيلية.</p>
              </div>
              <div className="flex items-center gap-2">
                <Select value={selectedReportType} onValueChange={setSelectedReportType}>
                  <SelectTrigger className="w-[260px] text-xs font-bold">
                    <SelectValue placeholder="اختر التقرير" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="by_supplier">1. تقرير المشتريات حسب المورد</SelectItem>
                    <SelectItem value="by_item">2. تقرير المشتريات حسب الصنف والتكلفة</SelectItem>
                    <SelectItem value="debt_aging">3. تقرير اعمار الديون للموردين (Aging Report)</SelectItem>
                    <SelectItem value="general">4. تقرير حركة الفواتير الإجمالية</SelectItem>
                  </SelectContent>
                </Select>
                <Button size="sm" variant="outline" onClick={() => printDocument("تقرير المشتريات والموردين", reportData)}>
                  <Printer className="w-4 h-4" /> طباعة
                </Button>
              </div>
            </div>

            <div className="bg-card rounded-xl border overflow-hidden shadow-sm p-4">
        ...
        <Dialog open={isPrintModalOpen} onOpenChange={setIsPrintModalOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <Printer className="w-5 h-5 text-primary" /> معاينة المستند للطباعة
              </DialogTitle>
            </DialogHeader>
            <div className="p-6 border rounded-xl bg-white text-black space-y-4 font-sans dir-rtl text-xs">
              <div className="text-center border-b pb-4">
                <h2 className="text-xl font-bold">مطعم المذاق الراقي - نظام المشتريات</h2>
                <p className="text-sm font-semibold text-gray-600 mt-1">{printDoc?.title}</p>
                <p className="text-[11px] text-gray-500 mt-0.5">تاريخ الطباعة: {new Date().toLocaleString("ar-SA")}</p>
              </div>

              <div className="bg-gray-50 p-3 rounded-lg border text-xs font-mono space-y-1">
                <pre className="whitespace-pre-wrap font-mono text-[11px]">{JSON.stringify(printDoc, null, 2)}</pre>
              </div>
            </div>
            <DialogFooter>
              <Button onClick={() => { window.print(); setIsPrintModalOpen(false); }}>
                <Printer className="w-4 h-4 ml-1" /> طباعة المستند الآن
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
