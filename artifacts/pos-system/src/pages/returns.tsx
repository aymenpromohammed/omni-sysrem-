...
export default function Returns() {
  ...
  useEffect(() => {
    ...
    const invParam = urlParams.get("invoice") || urlParams.get("invoiceNumber") || urlParams.get("q") || urlParams.get("orderId");
    if (invParam) {
      setInitialInv(invParam);
      setInvoiceQuery(invParam);
      executeInvoiceLookup(invParam);
    }
  }, []);
  ...
  const executeInvoiceLookup = async (term: string) => {
    ...
    try {
      const rawData = await apiGet(`/api/orders/lookup?q=${encodeURIComponent(term.trim())}`);
      const data = Array.isArray(rawData) ? rawData[0] : rawData;
      if (!data || !data.id) {
        throw new Error(`لم يتم العثور على الفاتورة رقم "${term}"`);
      }
      setFoundOrder(data);
      ...
      toast({ title: "✅ تم استدعاء بيانات الفاتورة", description: `رقم الفاتورة: ${data.invoiceNumber} — إجمالي البنود: ${data.items?.length || 0}` });
    } catch (e: any) {
      setFoundOrder(null);
      toast({ 
        variant: "destructive", 
        title: "تنبيه الفواتير غير موجودة", 
        description: e.message || "لم يتم العثور على الفاتورة. يرجى التأكد من ادخال رقم صحيح كـ 1001 أو 1002 وإعادة المحاولة." 
      });
    } finally {
                ...
                <div>
                  ...
                  <Select value={safeId} onValueChange={setSafeId}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="2">102 - صندوق الإدارة</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
