...
export default function Pos() {
  ...
  const browserPrintFullOrder = async () => {
    // تطبيق إعدادات الطابعة ديناميكياً قبل الطباعة (رأس طباعة حراري متزن)
    const ps = printerSettings;
    const styleId = "__pos-dynamic-print__";
    document.getElementById(styleId)?.remove();
    if (ps) {
      const pw = ps.paperWidth ?? 80;
      // رأس طباعة الـ 80mm الحراري عرضه 72mm، ورأس الـ 58mm عرضه 48mm لضمان عدم طمس الهامش الأيسر
      ...
      el.textContent = `
        ...
        .receipt-slip, .dept-receipt-slip {
          font-size: ${ps.fontSize ?? 10}px !important;
          line-height: ${1 + (ps.lineSpacing ?? 2) / 10} !important;
          padding: ${ps.topMargin ?? 1}mm ${ps.rightMargin ?? 1.5}mm ${ps.bottomMargin ?? 1}mm ${ps.leftMargin ?? 1.5}mm !important;
          box-sizing: border-box !important;
          width: 100% !important;
        }
      `;
