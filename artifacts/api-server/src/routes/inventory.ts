...
router.get("/inventory/summary", (_req, res) => {
  try {
    ...
    const auditLogs = db.prepare(`SELECT * FROM audit_logs WHERE action LIKE '%مخزن%' OR action LIKE '%إلغاء%' OR action LIKE '%مرتجع%' ORDER BY id DESC LIMIT 50`).all();
...
router.post("/inventory/cancel-operation", (req, res) => {
  ...
  db.transaction(() => {
    ...
    if (operationType === "issue_voucher") {
