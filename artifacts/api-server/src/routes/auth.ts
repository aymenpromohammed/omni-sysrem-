...
function checkLicenseStatus() {
  try {
    ...
    if (totalLicensesCount === 0) {
      // Auto-provision standard valid enterprise license ONLY if database is completely fresh
      try {
        db.prepare(`
    ...
    if (!lic.active) {
      return {
        ...
        reason: "تم انتهاء فترة صلاحيات ترخيص استخدام النظام يرجى التواصل مع إدارة النظام من أجل ترخيص الاستخدام"
      };
    }
    ...
    if (!deviceCheck) {
      ...
      if (activeDevicesCount < (lic.devices_limit || 10)) {
        ...
        return { 
          ...
          reason: "يمنع منعاً باتاً استخدام النظام بدون ترخيص. هذا الجهاز غير مصرح له بتشغيل النظام، يرجى التواصل مع إدارة إتقان سوفت (777146387)."
        };
      }
    } else {
      try {
...
router.post("/auth/login", (req, res) => {
  ...
  // License status check: If license is expired or device is unauthorized, developer and admin can still login to fix/renew license
  if (cleanUsername.toLowerCase() !== "developer" && cleanUsername.toLowerCase() !== "admin") {
    ...
    if (licenseStatus.blocked) {
      res.status(403).json({
        ...
        message: licenseStatus.reason || "يمنع منعاً باتاً استخدام النظام بدون ترخيص. يرجى التواصل مع إدارة إتقان سوفت 777146387"
      });
      return;
    }
  }
