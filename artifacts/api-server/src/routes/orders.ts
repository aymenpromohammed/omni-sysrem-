...
router.post("/orders", (req, res) => {
  ...
  const { items, paymentMethod, subtotal, discount, tax, total, cashAmount, cardAmount, customerId, userId, note, orderType, tableNumber, safeId } = req.body;
  if (!items?.length) { res.status(400).json({ error: "لا توجد منتجات" }); return; }

  const effectiveUserId = userId ?? authUser.id;
  const effectiveSafeId = safeId ?? 1;

  const r = db.prepare(`
    INSERT INTO orders (invoice_number, subtotal, discount, tax, total, payment_method, cash_amount, card_amount, customer_id, user_id, note, order_type, table_number, safe_id, created_at)
    VALUES (
      (SELECT CAST(COALESCE(MAX(CAST(invoice_number AS INTEGER)), 0) + 1 AS TEXT) FROM orders WHERE date(created_at, 'localtime') = date('now', 'localtime')),
      ?,?,?,?,?,?,?,?,?,?,?,?,?,?
    )
  `).run(
    subtotal ?? 0,
  ...
  const orderData = db.prepare("SELECT invoice_number FROM orders WHERE id=?").get(orderId) as { invoice_number: string };
  ...
  try {
    ...
    const debitAccount = isCredit ? "11200" : "11100"; // AR or Safe cash
