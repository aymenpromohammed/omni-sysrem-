...
router.post("/orders", (req, res) => {
  ...
  const maxRow = db.prepare("SELECT MAX(id) as maxId, COUNT(*) as c FROM orders").get() as { maxId: number; c: number };
  const nextVal = (maxRow?.maxId || maxRow?.c || 0) + 1;
  const invoiceNumber = String(1000 + nextVal);
...
router.get("/orders/lookup", (req, res) => {
  ...
  const cleanQ = qStr.replace(/^(inv-?|#)/i, "").replace(/^0+/, "") || qStr;
