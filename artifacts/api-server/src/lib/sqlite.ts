...
function runMigrations() {
  ...
  // Convert old INV-0001 style invoice numbers to clean plain integers (1001, 1002, ...)
  try {
    db.exec(`
      UPDATE orders 
      SET invoice_number = CAST(1000 + id AS TEXT) 
      WHERE invoice_number LIKE 'INV-%' OR invoice_number LIKE '#%' OR invoice_number LIKE '0%';
    `);
    db.exec(`
      UPDATE returns 
      SET invoice_number = CAST(1000 + COALESCE(order_id, 1) AS TEXT) 
      WHERE invoice_number LIKE 'INV-%' OR invoice_number LIKE '#%';
    `);
  } catch (e) {
